...
$BuildRoot = Join-Path $ProjectRoot ".build\agent-v2"
$PublishDir = Join-Path $BuildRoot "publish"
$TempConfig = Join-Path $BuildRoot "instance-config.json"
$DownloadsExe = Join-Path $env:USERPROFILE "Downloads\WisdomTI-Agent-Setup.exe"
$PublicDir = Join-Path $ProjectRoot "public\downloads"
$PublicExe = Join-Path $PublicDir "WisdomTI-Agent-Setup.exe"
...
try {
    Step "Compilando Wisdom TI Agent 2.0"
    ...
    $PublishedExe = Join-Path $PublishDir "WisdomTI.Agent.exe"

    if (-not (Test-Path $PublishedExe)) {
        Fail "WisdomTI.Agent.exe nao foi gerado."
    }
